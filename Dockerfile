...
# then log output of file /code/bin/ssgg
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/ssgg \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/ssgg | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/ssgg /usr/bin/ssgg
