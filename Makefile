...
HTTPS_GIT := github.com/suyog-bhat/ssgg.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=ssgg \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=ssgg \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/ssgg ./cmd/ssgg
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/ssgg.exe ./cmd/ssgg
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/ssgg
...
draw-deps:
	...
	@goviz -i ./cmd/ssgg -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c ssgg --local -f chains.yaml
endif
...
set-testnet-configs:
	ssgg config set client chain-id chainid-1
	ssgg config set client keyring-backend test
	ssgg config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | ssgg keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | ssgg keys add acc1 --recover`
